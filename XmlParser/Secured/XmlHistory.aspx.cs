...
namespace XmlParser.Secured
{
    public partial class XmlHistory : CommonFunctions
    {
        ...
        protected bool IsInEditMode
        {
            ...
            set { this._isEditMode = value; }
 
        }
        protected void Page_Load(object sender, EventArgs e)
        {
            this.Form.DefaultButton = this.lnkBtnSearch.UniqueID;
        ...
        private void BindGrid()
        {
            ...
            {
                HideControls(false);
                lblMsg.Text = "No records found. " + Environment.NewLine +
                              "If you just added the channel please wait till the process script runs at 7:00 AM.";
        ...
        protected void gvFiles_OnRowEditing(object sender, GridViewEditEventArgs e)
        {
        ...
        protected void lnkEdit_OnClick(object sender, EventArgs e)
        {
            ...
            lblMsg.Text = string.Empty;
            BindGrid();
            lnkEditSubmit.Visible = true;
            lnkEdit.Enabled = false;
        }

        protected void lnkEditSubmit_OnClick(object sender, EventArgs e)
        {
            for (var i = 0; i < gvFiles.Rows.Count; i++)
            {
                if (gvFiles.Rows[i].RowType == DataControlRowType.DataRow)
                {
                    var txtChannelId = (TextBox) gvFiles.Rows[i].FindControl("txtChannelId");

                    if (txtChannelId != null)
                    {
                        var srno = Convert.ToInt32(gvFiles.DataKeys[gvFiles.Rows[i].RowIndex].Values[1]);
                        var value = txtChannelId.Text;
                        using (var dataContext = new EPGDataModel())
                        {
                            try
                            {
                                dataContext.Database.ExecuteSqlCommand("UPDATE XmlImport SET ChannelId='" + value + "' WHERE Srno='"+ srno +"'");
                            }
                            catch (Exception ex)
                            { }
                        }
            ...
            IsInEditMode = false;
            lnkEditSubmit.Visible = false;
            lnkEdit.Enabled = true;
            lblMsg.Text = "Channel Id(s) updated successfully!";
            BindGrid();
        }

        private void HideControls(bool isVisible)
        {
            gvFiles.Visible = isVisible;
            ucPaging.Visible = isVisible;
            lnkBtnDownload.Visible = isVisible;
            lnkEdit.Visible = isVisible;
            spanInfo.Visible = isVisible;
        }
    }
}