...
namespace XmlParser.Secured
{
    public partial class ChannelSelection : Page
    {
        ...
        protected void btnManualAdd_OnClick(object sender, EventArgs e)
        {
            using (var dataContext = new EPGDataModel())
            {
                try
                {
                    dataContext.ActiveChannels.Add(new ActiveChannel
                    {
                        SourceId = 4,//4 is a dummy source for custom channels.
                        ChannelName = txtManualChannel.Text,
                        IsManuallyAdded = true,
                        IsActive = true
                    });

                    dataContext.SaveChanges();
                    lblMsg.Text = "Channel \"" + txtManualChannel.Text + "\" added successfully";
                    txtManualChannel.Text = string.Empty;
                    PopulateInactive(Convert.ToInt64(ddlSourceXml.SelectedValue));
                    PopulateActive(Convert.ToInt64(ddlSourceXml.SelectedValue));
                }
                catch (Exception)
