...
namespace XmlParser
{
    public partial class Index : Page
    {
        ...
        readonly List<string> _timetableXmlList = new List<string>();
            ...
            using (var dataContext = new EPGDataModel())
            {
                ...
                if (channelList.Count > 0)
                {
                    try
                    {
                        ...
                        {
                            _timetableXmlList.AddRange(GenerationEngine.GenerateTimeTableXml(channel));
                        }
                    }
                ...
                foreach (var entry in _timetableXmlList)
                {
                    ...
                    if (fileName != null)
                    {
                        ...
                        if (dbEntry != null)
                        {
                            ...
                            {
                                generatedId = CommonFunctions.GetChannelId(Path.GetFileName(xmlFile));
                            }
        ...
        private void BindGrid()
        {
            using (var dataContext = new EPGDataModel())
            {
                gvXMLSource.DataSource = dataContext.SourceURLs.Where(s => s.IsActive).ToList();
